...
class Discriminator_v3:
    ...
    def __call__(self, x, batch_size=100, reuse=None, train_phase = True, outdim = 1):
        ...
        #for i in xrange(len(self.hidden_dims)):
            #net = slim.fully_connected(net, self.hidden_dims[i], scope='dis_fc{}'.format(i), activation_fn=tf.tanh, reuse=reuse)
        logits = slim.fully_connected(lth_layer, outdim, scope='logits', activation_fn=tf.nn.sigmoid, reuse=reuse)
